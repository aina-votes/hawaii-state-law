...
| 2 | **Hawaiʻi Constitution** | all state and county law | `lrb.hawaii.gov/constitution/` (200, 236KB) | **yes** — 18 articles enumerated | **COMPLETE: 172 sections (2026-07-26, commit 88ee8b8)** — TOC-assertion clean, every constitutional edge joins. *(This row read "no / no" until 2026-07-27 — stale by a day and a half after the ingest landed. The ledger is only as good as the commit that updates it.)* |
...
| 4 | **Session Laws of Hawaiʻi** (SLH) | statewide, incl. uncodified text | `capitol.hawaii.gov/slh/` (200); acts reached through the sibling `hi-leg-db`, which holds every measure 2008-2026 | **yes, 2008 forward** — **4,747 acts** identified from the Capitol's own disposition line, each joined to the bill draft that became law | **PARTIAL, and this is the new frontier (2026-07-27):** every act is joined to the HRS sections it changed (`act_bridge.py` → `relation IN (amended_by, repealed_by, added_by)`, dst `slh:<year>:<act>`), and the enacted text's amendment markup is parsed into what each act deleted and inserted. **Not read: pre-2008 acts, and the uncodified provisions** — effective dates, sunsets, applicability, findings — which live in the act and never enter HRS. Two attestations carried unmerged: `act_text` (the act's operative sentence) and `bill_refs` (hi-leg-db's citation extraction). |
...
| 7 | **Agency opinions and guidance** | persuasive to binding-in-practice | CSC AOs: `ags.hawaii.gov/campaign/legal-resources/advisory-opinions/` (14 PDFs, AO10-01–AO26-02). AG opinions: `lrb.hawaii.gov/wp-content/uploads/AGOpinions.pdf` — **LRB-published, which is why every ag.hawaii.gov path 404s** | indexes located 2026-07-25; CSC layer is **57 entries, not 14 PDFs** (13 PDFs 2010-2026 + 44 HTML 1996-2009) | **PARTIAL: 56 CSC advisory opinions ingested, 49 with full text (2026-07-26, commit f91b97f)** — 7 are scans awaiting OCR, AO 06-02's page 404s though indexed. **AG opinions still unread**: the PDF is downloaded and hashed, never parsed. *(This row read "no" until 2026-07-27.)* |
